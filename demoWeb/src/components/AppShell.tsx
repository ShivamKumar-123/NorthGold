...
export default function AppShell() {
  ...
  // `useRequireAuth` on each page handles the redirect; this only guards
  // against rendering a shell with no account behind it.
  if (!user) return null;
    ...
    <div className="flex min-h-screen">
      ...
      >
        <div className="flex h-[74px] shrink-0 items-center justify-between gap-2 border-b border-border px-4">
          <Link to="/dashboard" className="flex items-center transition-transform duration-300 hover:scale-[1.03]">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-[74px] items-center gap-3 border-b border-border bg-bg/85 px-4 backdrop-blur-xl lg:hidden">
          ...
          <Link to="/wallet" className="card ml-auto px-3 py-1.5 text-right">
        ...
        <footer className="tex-footer border-t border-border px-4 py-5 text-center text-xs text-text-dim sm:px-6">
          <span className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1">
            ...
            <Link to="/profile" className="hover:text-text-muted">Profile &amp; KYC</Link>
            <Link to="/wallet" className="inline-flex items-center gap-1 hover:text-text-muted">
