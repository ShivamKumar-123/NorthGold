 ...
 * people already use, and the actions it offers depend on who is looking:
 * anybody may reply, react, star, forward and copy; only the author may edit,
 * and only while the other side has not read it; deleting is whatever the
 * caller says it is.
 */
export default function ChatMessage({
  ...
  /** The message this one quotes, resolved by the caller so an edited original
   *  shows its current text. */
  quoted?: SupportMessage | null;
  mine: boolean;
  viewerId: string;
  canDelete: boolean;
  canEdit: boolean;
  ...
  const starred = (message.starred_by ?? []).includes(viewerId);
  const myReaction = Object.entries(message.reactions ?? {})
    .find(([, ids]) => ids.includes(viewerId))?.[0] ?? '';
  const counts = Object.entries(message.reactions ?? {}).filter(([, ids]) => ids.length);
    ...
    >
      <div className="relative max-w-[85%]">
        ...
        >
          ...
          {quoted && (
            <button
              onClick={(e) => { e.stopPropagation(); onJumpTo?.(quoted.id); }}
            ...
            >
              <span className="block text-[10px] font-semibold text-accent">
                {quoted.sender === 'admin' ? quoted.author_name || 'Support' : 'Member'}
              </span>
              <span className="line-clamp-2 block text-[11px] text-text-muted">{quoted.body}</span>
            </button>
          )}
        ...
        {counts.length > 0 && (
          <div className={`-mt-1.5 flex flex-wrap gap-1 ${mine ? 'justify-end pr-2' : 'pl-2'}`}>
            {counts.map(([emoji, ids]) => (
              <button
                key={emoji}
                onClick={() => onReact?.(message.id, emoji)}
                title={ids.includes(viewerId) ? 'Remove your reaction' : 'React'}
              ...
              >
                <span>{emoji}</span>
                {ids.length > 1 && <span className="text-text-muted">{ids.length}</span>}
              </button>
            ))}
          </div>
        )}
        ...
        {menuOpen && !editing && (
          ...
          >
                ...
                {QUICK_REACTIONS.map((emoji) => (
                  <button
                    ...
                    className={`grid h-7 w-7 place-items-center rounded-full text-base transition
                                hover:scale-125 ${myReaction === emoji ? 'bg-accent/25' : ''}`}
                  >
                    {emoji}
                  </button>
                ))}
            ...
            {onStar && (
              <MenuItem
                icon={<Star size={14} className={starred ? 'fill-accent text-accent' : ''} />}
      ...
      <div className={`flex items-center gap-2 text-[10px] text-text-dim ${mine ? 'flex-row-reverse' : ''}`}>
        ...
        {starred && <Star size={10} className="fill-accent text-accent" />}
