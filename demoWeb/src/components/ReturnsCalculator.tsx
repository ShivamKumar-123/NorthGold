...
import { planForAmount, projectedSchedule } from '@/lib/store';
import type { Projection, RoiPlan } from '@/lib/types';
 ...
 *
 * The schedule comes from the same `projectedSchedule` the store uses when it
 * actually pays someone, so the number a prospect sees here is produced by the
 * code that will later credit their wallet — not a second implementation that
...
export default function ReturnsCalculator({ plans }: { plans: RoiPlan[] }) {
  ...
  useEffect(() => {
    ...
    // Debounced so the schedule does not rebuild on every keystroke while
    // someone is still typing an amount.
    setLoading(true);
    const timer = setTimeout(() => {
      const plan = planId ? plans.find((p) => p.id === planId) ?? null : planForAmount(value);
      if (!plan) {
        setProjection(null);
        setError(`No plan covers ${money(value)}. The entry tier starts at ${money(plans[0]?.min_amount ?? 0)}.`);
        setLoading(false);
        return;
      }
      const schedule = projectedSchedule(value, plan);
      ...
      setProjection({
        ...
        total_return: total,
        total_return_percent: value ? (total / value) * 100 : 0,
        returns_principal: true,
        schedule,
      });
      setError('');
      setLoading(false);
  ...
  const peak = projection
    ? projection.schedule.reduce((m: number, r: Projection['schedule'][number]) => Math.max(m, r.payout), 0)
    : 0;
            ...
            <div className="space-y-5">
              ...
              <div className="rounded-2xl border border-border bg-bg-sunken/60 p-5">
                ...
                <div className="flex h-28 items-end gap-1.5">
                  {projection.schedule.map((row: Projection['schedule'][number]) => (
                    <div
                      key={row.month}
