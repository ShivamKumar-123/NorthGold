...
import { getPlans, invest, planForAmount } from '@/lib/store';
import { totalReturnPercent, type RoiPlan } from '@/lib/types';

type Investment = q.InvestmentView;
type InvestmentSummary = q.InvestmentSummaryView;
...
export default function InvestmentsPage() {
  ...
  const load = useCallback(() => {
    if (!user) return;
    // Synchronous: the store is already in memory. The loading flag survives
    // because the screen's empty states are keyed off it.
    setSummary(q.investmentSummary(user.id));
    setInvestments(q.investments(user.id));
    setPlans(getPlans());
    setLoading(false);
  }, [user]);
  ...
  async function openDetail(investment: Investment) {
    try {
      // Every payout is already on the record the list rendered from.
      ...
      <div className="mt-8">
        {investments.length === 0 ? (
          <EmptyState
            ...
            action={<Link to="/wallet" className="btn-primary">Deposit funds</Link>}
          />
        ) : (
...
function CreateInvestmentModal({
  ...
  function submit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      invest(userId, value, planId || undefined);
      setAmount('');
      setPlanId('');
      onDone('Investment created. Your first return is due in one month.');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Could not create the investment.');
    } finally {
      setSubmitting(false);
    }
  }
        ...
        {chosen && (
          <div className="rounded-lg border border-border bg-bg-elevated p-4">
            <div className="flex items-center justify-between">
              ...
              <p className="tabular-nums text-gold">{num(totalReturnPercent(chosen), 2)}% total</p>
            </div>
            <div className="mt-3 grid grid-cols-6 gap-1.5 text-center text-[10px]">
              {chosen.months.map((percent, i) => (
                <div key={i} className="rounded bg-bg p-1.5">
                  <p className="text-text-dim">M{i + 1}</p>
                  <p className="tabular-nums text-success">{num(percent, 2)}%</p>
                </div>
              ))}
            </div>
            {value > 0 && (
              <p className="mt-3 border-t border-border pt-2.5 text-sm text-text-muted">
                ...
                <strong className="text-success">
                  {money((value * totalReturnPercent(chosen)) / 100)}
                </strong>{' '}
