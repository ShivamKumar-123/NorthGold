...
/* ─────────────────────────────────────────────────────────────────
   Shape adapter. The only difference between this file and its twin in the
   demo: the API serialises a plan's months as `{ percent }` rows and carries
   the total as its own field, where the demo stores plain percentages.
   Everything below is identical in both, so the two stay in step. */
const monthsOf = (plan: RoiPlan): number[] => plan.months.map((row) => Number(row.percent));
const totalOf = (plan: RoiPlan): number => Number(plan.total_return_percent);
/* ─────────────────────────────────────────────────────────────────────── */
