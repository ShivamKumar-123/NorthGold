...
export const metadata: Metadata = {
  title: 'Returns calculator — NorthGold',
  description:
    'Enter an amount and read the whole term before you commit: every month, its rate, its payout and the running total — the same schedule the platform pays.',
};

// Same reason as the landing page: the API is not reachable during `next
// build`, so a prerendered page would ship with an empty plan list.
...
async function fetchPlans(): Promise<RoiPlan[]> {
  try {
    const res = await fetch(`${API_BASE}/investments/plans/`, { next: { revalidate: 60 } });
    if (!res.ok) return [];
    return (await res.json()) as RoiPlan[];
  } catch {
    // The API being unreachable must degrade the page, not blank it — the
    // calculator renders its own empty state.
    return [];
  }
}
