...
export const metadata: Metadata = {
  title: 'Contact — NorthGold',
  description: 'Reach the NorthGold desk by WhatsApp, phone or email, or come to the counter.',
};

// Rendered per request: the numbers below are admin-editable settings, and a
// prerendered page would ship whatever they were at build time.
...
async function fetchSettings(): Promise<PublicSettings> {
  try {
    const res = await fetch(`${API_BASE}/core/public-settings/`, { next: { revalidate: 60 } });
    if (!res.ok) return {};
    return (await res.json()) as PublicSettings;
  } catch {
    // The desk details are useful even stale; an unreachable API must not
    // blank the page.
    return {};
  }
}
